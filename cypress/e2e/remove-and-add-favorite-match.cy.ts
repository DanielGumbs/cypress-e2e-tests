describe('Login Form', () => {
    it('Clicks on a favorite matches', () => {
        ...
        cy.screenshot('after_add_a_favorite-3');
        cy.screenshot('before-remove-a-favorite-3');
        cy.get('img[src="/favorite-1.svg"]').closest('button').first().click();
        cy.contains('button', '12-9').click();
        cy.screenshot('after-remove-a-favorite-3');
        cy.screenshot('before_add_a_favorite-3');
